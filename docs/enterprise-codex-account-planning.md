...
轻量脚本、偶发代码辅助、学习和一般办公可从 Plus 评估；高频多文件、复杂调试或长任务持续受限时先比较 Pro 5x；只有单个使用者全天重度、多项目并行且 5x 仍反复影响交付时，可评估 Pro 20x；新开需在本人升级页选择 Pro 20x 并进入付款页面；续订需仍显示 20x 且原非 App Store／Google Play 付款，包括已逾期但 20x 未消失。实际限制、模型和消耗方式会变化，最终以该账号的官方 Usage 页面为准。
...
可以按每名实际使用者一个独立账号进行规划；20x 新开／续订统一 ¥1298；新开逐账号确认本人升级页可选 Pro 20x 并进入付款页面，续订确认仍显示 20x 且原非 App Store／Google Play 付款（含已逾期但 20x 未消失）。符合条件可直接下单；商店订阅或无法判断的账号先咨询。企业已有账号时优先在自有账号上开通；账号准备不足时，可在授权和官方规则允许的前提下协助准备一人一号的独立账号。不得把一个个人 Pro 账号多人共享。
