...
| 单人全天长任务、多项目并行，5x 仍持续不足 | Pro 20x 全新充值或条件续费 | 逐账号核对受理条件，个人套餐不是多人席位 |
...
轻量脚本、偶发代码辅助、学习和一般办公可从 Plus 评估；高频多文件、复杂调试或长任务持续受限时先比较 Pro 5x；只有单个使用者全天重度、多项目并行且 5x 仍反复影响交付时，可评估 Pro 20x；新号、无会员，或过期后不再显示 20x 的账号可选全新充值，满足原条件的已有 20x 可选续费。实际限制、模型和消耗方式会变化，最终以该账号的官方 Usage 页面为准。
...
可统一对接 Plus、Pro 5x 与 Pro 20x 全新充值或条件续费的岗位分层、批次交付、官方账号内验真、订单售后与实际交易发票。需要集中工作区、SSO、SCIM、官方企业合同或 OpenAI 官方发票时，则应优先走 OpenAI Business / Enterprise 官方路线。
...
可以按每名实际使用者一个独立账号进行规划；新增 20x 需求可逐账号核对 ¥1529 全新充值，符合原条件的已有 20x 可选 ¥1269 续费；仍有 Plus/5x 或商店订阅的账号先咨询确认。企业已有账号时优先在自有账号上开通；账号准备不足时，可在授权和官方规则允许的前提下协助准备一人一号的独立账号。不得把一个个人 Pro 账号多人共享。
